...
function TransactionModal({ type, year, month, exchangeRate, editTx, onClose }: {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    // - Single proc con materiales cargados: materiales actuales + honorarios + costos_fijos
    //   (así editar/eliminar materiales en el form siempre se refleja en el costo op.)
    ...
    } else if (form.procedure_id && resolvedMaterials) {
      const apptTreatment = apiTreatments.find((t) => t.procedure_catalog_id === form.procedure_id);
      const materialCost = calcMaterialsCost(resolvedMaterials);
      if (apptTreatment) {
        const profFees = (apptTreatment.professional_fee_per_hour || 0) * (apptTreatment.total_hours || 0);
        const fixedCosts = apptTreatment.fixed_costs || 0;
        opCostOverride = Math.round((materialCost + profFees + fixedCosts) * 100) / 100;
      } else {
        opCostOverride = materialCost;
      }
    }
...
export default function FinancesPage() {
  ...
  return (
    <div className="p-6 space-y-6 max-w-7xl mx-auto">
      {/* ── Barra sticky ── */}
      <div className="sticky top-0 z-20 -mx-6 px-6 -mt-6 pt-4 pb-4 bg-slate-50/95 dark:bg-gray-900/95 backdrop-blur-sm border-b border-slate-200 dark:border-gray-800">
        <div className="flex items-center justify-between flex-wrap gap-3">
          <div>
          ...
          <div className="flex items-center gap-3 flex-wrap">
            ...
            <button
              ...
              title={showSummary ? "Ocultar resumen financiero" : "Mostrar resumen financiero"}
              className="flex items-center gap-2 rounded-xl border border-slate-200 dark:border-gray-600 bg-white dark:bg-gray-800 px-3 py-2 text-sm font-medium text-slate-500 dark:text-gray-400 hover:bg-slate-100 dark:hover:bg-gray-700"
            >
